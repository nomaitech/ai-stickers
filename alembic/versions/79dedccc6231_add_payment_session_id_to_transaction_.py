...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column(
        "transactions", sa.Column("payment_session_id", sa.Integer(), nullable=True)
    )
    ...
    op.create_index(
        "idx_transactions_payment_session_id", "transactions", ["payment_session_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_transactions_payment_session_id", "transactions", type_="foreignkey"
    )
